...
directive
 : command
   (   codeblock
     ...
     | numberRanges
     | properties
     | byteSizeArg        // Added support for BYTE_SIZE
     | timeDurationArg    // Added support for TIME_DURATION
   )*?
  ;
...
value
 ...
 | byteSizeArg          // Added BYTE_SIZE support
 | timeDurationArg      // Added TIME_DURATION support
 ;
...
BYTE_SIZE: [0-9]+ (KB | MB | GB | B);  // Matches "10KB", "20MB", etc.
TIME_DURATION: [0-9]+ (ms | s | m | h);  // Matches "150ms", "1s", "5m", "2h"
