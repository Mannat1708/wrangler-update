...
public final class RecipeVisitor extends DirectivesBaseVisitor<RecipeSymbol.Builder> {
  ...
  public RecipeSymbol.Builder visitCommand(DirectivesParser.CommandContext ctx) {
    ...
    builder.addToken(new DirectiveName(directiveName));

    // Handle new parse-byte-size and parse-duration directives
    if ("parse-bytesize".equalsIgnoreCase(directiveName)) {
      // Assume the next argument is a size string (e.g., "10MB", "2GB")
      String sizeStr = ctx.getChild(1).getText();
      builder.addToken(new Text(sizeStr)); // Handle byte size as Text token
    } else if ("parse-duration".equalsIgnoreCase(directiveName)) {
      // Assume the next argument is a duration string (e.g., "10s", "2m")
      String durationStr = ctx.getChild(1).getText();
      builder.addToken(new Text(durationStr)); // Handle duration as Text token
    }
